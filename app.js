...
function remoteQty(n,measure){
 const cn=remoteCanonical(n)||normalizeName(n), m=String(measure||'').toLowerCase();
 const meta=canonMeta(cn); let q=parseFloat((m.match(/\d+(?:[.,]\d+)?/)||[])[0]?.replace(',','.')||'');
 if(!q) q=meta.unit==='pz'?1:({grain:80,meat:150,fish:150,egg:2,legume:150,dairy:50,vegetable:150,fruit:150,oil:10}[meta.group]||50);
 if(/kg|kilogram/.test(m))q*=1000;
 if(/lb|pound/.test(m))q*=453.6;
 if(/oz|ounce/.test(m))q*=28.35;
 if(/cup/.test(m)&&meta.unit!=='pz')q*=({grain:70,vegetable:120,fruit:140,dairy:200,legume:170}[meta.group]||120);
 if(/tablespoon|tbsp/.test(m)&&meta.unit!=='pz')q*=({oil:13,dairy:15}[meta.group]||12);
 if(/teaspoon|tsp/.test(m)&&meta.unit!=='pz')q*=4;
 return [Math.max(.1,q),meta.unit==='pz'?'pz':(meta.unit==='ml'?'ml':'g')];
}
function normalizeRemoteRecipe(r){
 const arr=(r.ingredients||[]).map(([n,q,u])=>{
   const cn=remoteCanonical(n); if(!cn)return [null,q,u];
   const qq=remoteQty(n,q); return [cn,qq[0],qq[1]];
 }).filter(x=>x[0]);
 const steps=(r.sourceInstructions||'').split(/\n+/).map(x=>x.trim()).filter(Boolean).slice(0,6);
 return {...r,ingredients:arr,steps,sourceUrl:r.sourceUrl||'',diff:2,time:30};
}
function remoteCovered(r,pantry){
 return normalizeRemoteRecipe(r).ingredients.every(([n,q,u])=>availableFor(n,pantry)>=baseAmount(q,u)-0.01);
}
function qaRecipe(r){
 if(!r||!r.name||!Array.isArray(r.ingredients)||r.ingredients.length<3)return false;
 if(r.remote&&!r.sourceUrl)return false;
 if(r.remote&&(!r.sourceInstructions||r.sourceInstructions.length<120))return false;
 if(r.ingredients.some(([n,q])=>!n||!q||q<=0))return false;
 return true;
}
function groceryEstimatedCost(pantry){if(state.budgetTarget>0&&state.budgetPreset)return Number(state.budgetTarget.toFixed(2));return Number(pantry.reduce((sum,i)=>{if(i.price>0)return sum+i.price;const meta=canonMeta(i.name);return sum+(meta.cost||0)*(baseAmount(i.qty,i.unit)/100)},0).toFixed(2))}
...
function microDaySignals(pair){
 ...
 return {
  'Vitamina C':['peperoni','broccoli','pomodori','limone','spinaci'].some(x=>names.has(x)),
  'Folati':['spinaci','broccoli','ceci cotti','lenticchie cotte','fagioli cannellini'].some(x=>names.has(x)),
  'Calcio':['ricotta','yogurt greco','feta','parmigiano','mozzarella'].some(x=>names.has(x)),
  'Ferro':['lenticchie cotte','ceci cotti','fagioli cannellini','spinaci','carne','uova'].some(x=>names.has(x)),
  'Omega-3':['salmone','sgombro','sardine','alici','noci'].some(x=>names.has(x)),
  'Potassio':['patate','spinaci','broccoli','ceci cotti','lenticchie cotte','banane'].some(x=>names.has(x))
 };
}
function nutritionDay(pair){
 const ms=pair.map(m=>recipeMacroSummary(m.recipe)); const total=ms.reduce((a,x)=>({kcal:a.kcal+x.kcal,p:a.p+x.p,c:a.c+x.c,f:a.f+x.f,fiber:a.fiber+x.fiber}),{kcal:0,p:0,c:0,f:0,fiber:0});
 ...
 const msig=microDaySignals(pair); const signals={vegetables:['broccoli','spinaci','zucchine','carote','peperoni','pomodori','melanzane','lattuga'].filter(x=>names.has(x)).length,fruit:['mela','banana','arance','frutta'].filter(x=>names.has(x)).length,legumes:[...names].some(n=>canonMeta(n).group==='legume'),omega3:msig['Omega-3'],calcium:msig['Calcio'],iron:msig['Ferro'],folate:msig['Folati'],vitC:msig['Vitamina C'],potassium:msig['Potassio']};
 const macroOK=(pct.c>=45&&pct.c<=60)&&(pct.f>=20&&pct.f<=35); const microOK=signals.vegetables>=2&&(signals.legumes||signals.omega3||signals.calcium||signals.iron); const ok=macroOK&&microOK;
 const score=Math.max(35,Math.min(100,Math.round(70+(signals.vegetables*5)+(signals.legumes?6:0)+(signals.omega3?6:0)+(signals.calcium?5:0)+(signals.iron?4:0)-Math.abs(pct.c-52)-Math.abs(pct.f-28))));
 return {total,pct,signals,score,ok:score>=75};
}
async function buildPlan(){
 ...
 let remotePool=[];
 if(window.GNAM_REMOTE){try{remotePool=(await window.GNAM_REMOTE.getLiveRecipePool(source)).map(normalizeRemoteRecipe).filter(qaRecipe).filter(r=>Number(r.diff||2)<=Number(state.difficulty||2));}catch{remotePool=[]}}
 const eligibleRemote=remotePool.filter(r=>remoteCovered(r,source));
 const curated=[...localPool,...eligibleRemote.filter(r=>!localPool.some(x=>x.name===r.name))];
 $('#recipePoolStatus').textContent=`🍽️ ${curated.length} ricette candidate · ${localPool.length} curate + ${eligibleRemote.length} da fonti online · verifica ingredienti attiva`;
 ...
 toast(meals.length<14?`Ho coperto ${Math.floor(meals.length/2)} giorni: non aggiungo ingredienti inventati.`:'14 pasti costruiti: ogni giorno è valutato anche come coppia pranzo+cena.');
}
...
function renderPlan(){
 ...
 g.innerHTML=byDay.map(day=>{const date=addDays(state.plan.start,day.d);const dn=day.meals.length===2?nutritionDay(day.meals):null;return `<article class="day-card"><div class="day-head"><div><b>${day.name}</b><small>${fmtDate(date)}</small></div><span class="day-score">${dn?`⚖ ${dn.score}/100`: `${day.meals.length}/2`}</span></div>${[0,1].map(slot=>{const m=day.meals.find(x=>x.slot===slot);return m?`<button class="meal-card ${slot?'dinner':''}" data-meal="${m.id}"><div class="meal-type">${slot?'CENA':'PRANZO'}</div><div class="meal-title">${escapeHtml(m.recipe.name)}</div><div class="meal-meta"><span>⏱ ${m.recipe.time||20}′</span><span>${m.recipe.kcal||520} kcal</span><span>${money(m.recipe.cost)}</span></div><div class="meal-bottom"><span>${m.recipe.sourceUrl?'✓ verificata':'curata'}</span><span class="cook-state ${state.cookLog[m.id]?'done':''}">${state.cookLog[m.id]?'✓ cucinato':'○ da fare'}</span></div></button>`:`<div class="meal-card empty-slot"><div class="meal-type">${slot?'CENA':'PRANZO'}</div><p>Non coperto dalla spesa</p></div>`}).join('')}<div class="day-nutrition"><b>${dn?'Giornata · '+(dn.ok?'OK':'da riequilibrare'):'Dati insufficienti'}</b>${dn?`<span>${dn.total.kcal} kcal · P ${dn.total.p}g · C ${dn.total.c}g · G ${dn.total.f}g · fibra ${dn.total.fiber}g</span><small>Macro: C ${dn.pct.c}% · G ${dn.pct.f}% · P ${dn.pct.p}% · micronutrienti: ${dn.signals.vegetables>=2?'✓ verdure':'△ verdure'} ${dn.signals.legumes?'✓ legumi':'△ legumi'} ${dn.signals.omega3?'✓ omega-3':'△ omega-3'} ${dn.signals.calcium?'✓ calcio':'△ calcio'} · ${dn.signals.vitC?'✓ vit.C':'△ vit.C'} ${dn.signals.folate?'✓ folati':'△ folati'} ${dn.signals.iron?'✓ ferro':'△ ferro'} ${dn.signals.potassium?'✓ K':'△ K'}</small>`:''}</div></article>`}).join('');
 ...
 status.innerHTML=`<b>${state.plan.meals.length}/14 pasti coperti</b> · ${Math.round(totalK/Math.max(1,state.plan.meals.length))} kcal medie/pasto · <b>${money(grocery)}</b> spesa stimata · ${money(totalMenuCost)} consumato dal menu`;
 $('#weeklySummary').innerHTML=`<div class="summary-stat"><b>${state.plan.meals.length}</b><span>pasti coperti</span></div><div class="summary-stat"><b>${money(grocery)}</b><span>costo della spesa</span></div><div class="summary-stat"><b>${money(totalMenuCost)}</b><span>valore ingredienti usati</span></div><div class="summary-stat"><b>${avgDayScore}/100</b><span>media equilibrio giornaliero</span></div><div class="summary-note">La spesa e il valore del menu sono ora separati: il primo è il costo dell'inventario, il secondo è il costo degli ingredienti effettivamente assegnati ai pasti. I micronutrienti mostrati sono segnali di copertura, non diagnosi nutrizionali.</div>`;
