...
async function buildPlan(){
 ...
 let remotePool=[];
 if(window.GNAM_REMOTE){try{remotePool=(await window.GNAM_REMOTE.getLiveRecipePool(source)).map(normalizeRemoteRecipe).filter(Boolean).filter(qaRecipe).filter(r=>Number(r.diff||2)<=Number(state.difficulty||2));}catch{remotePool=[]}}
 const eligibleRemote=remotePool.filter(r=>remoteCovered(r,source));
 const curated=[...localPool,...eligibleRemote.filter(r=>!localPool.some(x=>x.name.toLowerCase()===r.name.toLowerCase()))];
 $('#recipePoolStatus').textContent=`🍽️ ${curated.length} ricette candidate · ${localPool.length} curate + ${eligibleRemote.length} online verificate · nessuna ricetta mutilata`;
